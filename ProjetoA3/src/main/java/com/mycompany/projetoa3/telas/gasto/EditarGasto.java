...
public class EditarGasto extends JDialog {
    ...
    private Gasto gasto; // O gasto a ser editado
    ...
    public EditarGasto(Frame parent, Gasto gasto, Runnable onSuccess) {
        super(parent, "Editar Gasto", true); // true para modal
        this.gasto = gasto;
        this.onSuccess = onSuccess;
        initComponents();
        preencherCampos(); // Preenche os campos com os dados do gasto existente
    ...
    private void preencherCampos() {
        if (gasto != null) {
            ...
            tfValor.setText(String.valueOf(gasto.getValor()).replace(".", ","));
            
            // Selecionar a categoria atual
            if (gasto.getNomeCategoria() != null) {
                cbCategoria.setSelectedItem(gasto.getNomeCategoria());
            } else {
                ...
                List<Categoria> todasCategorias = CategoriaDAO.listarCategoriasPorTipo(1); // Busca apenas categorias de gasto
                for(Categoria c : todasCategorias) {
                    if(c.getIdCategoria() == gasto.getIdCategoria()) {
                        catAtual = c;
                        break;
                    }
                }
                ...
                } else if (cbCategoria.getItemCount() > 0 && !cbCategoria.getItemAt(0).equals("Nenhuma categoria de gasto")) {
                     cbCategoria.setSelectedIndex(0);
                } else if (cbCategoria.getItemCount() > 0 && cbCategoria.getItemAt(0).equals("Nenhuma categoria de gasto")){
                    // Não faz nada, mantém "Nenhuma categoria..." selecionado se for a única opção
                }
            }
            // Define o estado do checkbox com base no valor do objeto Gasto
            cbRecorrente.setSelected(gasto.isEhRecorrente());
        }
    }
    ...
    private void salvarEdicao() {
        ...
        // Atualizar os dados do objeto 'gasto' existente
        gasto.setDescricao(descricao);
        gasto.setValor(valor);
        gasto.setIdCategoria(categoriaSelecionada.getIdCategoria());
        // A data do gasto original (gasto.setDataGasto()) geralmente não é alterada na edição,
        // a menos que seja um requisito específico. Se precisar, adicione um JDateChooser.
        gasto.setEhRecorrente(ehRecorrente); // Atualiza o status de recorrência
        // O cpfUsuario já deve estar no objeto 'gasto' e não é tipicamente editado aqui.
        ...
        if (sucesso) {
            ...
            JOptionPane.showMessageDialog(this, "Erro ao atualizar gasto. Verifique o console para mais detalhes.", "Erro no Banco de Dados", JOptionPane.ERROR_MESSAGE);
        }
    }
}
