...
public class AdicionarRenda extends JDialog {
    ...
    public AdicionarRenda(Frame parent, String cpfUsuario, Runnable onSuccess) {
        super(parent, "Adicionar Renda", true); // true para modal
    ...
    private void initComponents() {
        ...
        btnSalvar.addActionListener(e -> salvarRenda());
    ...
    private void salvarRenda() {
        ...
        novaRenda.setDescricao(descricao);
        novaRenda.setValor(valor);
        novaRenda.setIdCategoria(categoriaSelecionada.getIdCategoria());
        novaRenda.setCpfUsuario(cpfUsuario);
        novaRenda.setDataRenda(new java.util.Date()); // Data atual do sistema
        novaRenda.setEhRecorrente(ehRecorrente); // Define o status de recorrência

        // Tenta inserir no banco de dados
        boolean sucesso = RendaDAO.inserirRenda(novaRenda);

        if (sucesso) {
            JOptionPane.showMessageDialog(this, "Renda adicionada com sucesso!", "Sucesso", JOptionPane.INFORMATION_MESSAGE);
            if (onSuccess != null) {
                onSuccess.run(); // Chama o callback para atualizar a tela de listagem (TelaRendas)
            }
            dispose(); // Fecha o diálogo de adição
        } else {
            JOptionPane.showMessageDialog(this, "Erro ao adicionar renda. Verifique o console para mais detalhes.", "Erro no Banco de Dados", JOptionPane.ERROR_MESSAGE);
        }
    }
}
